...
class CrossAttention(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.n_head = n_head
        self.head_dim = head_dim or (in_channels // n_head)
        assert (
            in_channels % n_head == 0
        ), f"in_channels {in_channels} not divisible by n_head {n_head}"
        self.scale = self.head_dim ** -0.5

        self.norm = GroupNorm(in_channels)
        self.q_proj = Conv1x1(in_channels, in_channels)
        self.k_proj = nn.Linear(cond_channels, in_channels)
        self.v_proj = nn.Linear(cond_channels, in_channels)
        self.out_proj = Conv1x1(in_channels, in_channels)
        nn.init.zeros_(self.out_proj.weight)
        nn.init.zeros_(self.out_proj.bias)
    ...
    def forward(self, x: Tensor, cond: Tensor) -> Tensor:
        ...
        n, c, h, w = x.shape
        x = self.norm(x)
        q = self.q_proj(x).view(n, self.n_head, self.head_dim, h * w)
        q = q.transpose(2, 3).contiguous().view(n * self.n_head, h * w, self.head_dim)
        # q: [batch_size * n_head, seq_len_q, head_dim]

        # Process cond to get k and v
        k = self.k_proj(cond).view(n, self.n_head, self.head_dim)
        v = self.v_proj(cond).view(n, self.n_head, self.head_dim)
        k = k.unsqueeze(2).contiguous().view(n * self.n_head, 1, self.head_dim)
        v = v.unsqueeze(2).contiguous().view(n * self.n_head, 1, self.head_dim)
        # k, v: [batch_size * n_head, seq_len_kv=1, head_dim]

        # Compute attention
        attn_weights = (q @ k.transpose(-2, -1)) * self.scale  # [n * n_head, seq_len_q, 1]
        attn_weights = F.softmax(attn_weights, dim=-1)
        attn_output = attn_weights @ v  # [n * n_head, seq_len_q, head_dim]
        attn_output = attn_output.view(n, self.n_head, h * w, self.head_dim)
        attn_output = attn_output.transpose(2, 3).contiguous().view(n, c, h, w)

        out = x + self.out_proj(attn_output)
        return out
...
class ResBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, cond_channels: int, attn: bool) -> None:
        ...
        self.cross_attn = CrossAttention(out_channels, cond_channels) if attn else None
    ...
    def forward(self, x: Tensor, cond: Tensor) -> Tensor:
        ...
        x = x + r
        x = self.cross_attn(x, cond) if self.cross_attn is not None else x
        x = self.attn(x)
        return x
