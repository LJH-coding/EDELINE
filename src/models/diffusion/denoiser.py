...
class Denoiser(nn.Module):
    ...
    def forward(self, noisy_next_obs: Tensor, sigma: Tensor, obs: Tensor, emb: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        model_output = self.inner_model(rescaled_noise, c_noise, rescaled_obs, emb)
        denoised = model_output * c_out + noisy_next_obs * c_skip
        return model_output, denoised

    @torch.no_grad()
    def denoise(self, noisy_next_obs: Tensor, sigma: Tensor, obs: Tensor, emb: Tensor) -> Tensor:
        _, d = self(noisy_next_obs, sigma, obs, emb)
    ...
    def compute_loss(self, batch: Batch, batch_emb):
        ...
        autoregressive_steps = self.cfg.num_autoregressive_steps
        seq_length = batch.obs.size(1)

        all_emb = batch_emb
        all_obs = batch.obs.clone()
        idx = torch.randint(0, seq_length - (n + 1 + autoregressive_steps) + 1, (1,)).item() 
        loss, valid_steps = 0, 0

        for i in range(idx, idx + 1 + autoregressive_steps):
            obs = all_obs[:, i : n + i]
            ...
            model_output, denoised = self(noisy_next_obs, sigma, obs, emb)
