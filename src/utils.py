...
def configure_opt(model: nn.Module, lr: float, weight_decay: float, eps: float, *blacklist_module_names: str) -> AdamW:
    ...
    decay = set()
    no_decay = set()
    whitelist_weight_modules = (nn.Linear, nn.Conv1d, nn.Conv2d, nn.LSTMCell, nn.LSTM, Mamba)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if any([fpn.startswith(module_name) for module_name in blacklist_module_names]):
                ...
                no_decay.add(fpn)
            elif "A_log" in pn or "D" in pn or "norm_f" in pn: #Mamba
                no_decay.add(fpn)
            elif "harmony_s2" in pn or "harmony_s1" in pn: #Harmony
                no_decay.add(fpn)
